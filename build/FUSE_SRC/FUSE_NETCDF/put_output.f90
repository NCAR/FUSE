SUBROUTINE PUT_OUTPUT(IPAR,IMOD,iSpat1,iSpat2,ITIM)
  ! ---------------------------------------------------------------------------------------
  ! Creator:
  ! --------
  ! Martyn Clark, 2007
  ...
  ierr = nf_inq_varid(ncid,'time',ivar_id); CALL handle_err(ierr)             ! get variable ID for time
  ierr = nf_put_var1_real(ncid,ivar_id,itim,tDat); CALL handle_err(ierr) ! write time variable
  ! close NetCDF file
  IERR = NF_CLOSE(NCID)
  ! ---------------------------------------------------------------------------------------

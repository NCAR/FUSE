...
INTEGER(I4B), DIMENSION(2)             :: FVAR        ! dimensions for summary statistics
INTEGER(I4B), DIMENSION(3)             :: PVAR        ! dimensions for probability distributions
...
IERR = NF_REDEF(NCID); CALL HANDLE_ERR(IERR)
 ...
 IERR = NF_DEF_DIM(NCID,'ord',SIZE(ORD_NSUBS),NORD_DIM); CALL HANDLE_ERR(IERR)
 ! define variables
 FVAR = (/NMOD_DIM,NPAR_DIM/)            ! dimensions for fixed output (parameters)
 DO IVAR=1,NSUMVAR
  IERR = NF_DEF_VAR(NCID,TRIM(XNAME(IVAR)),NF_REAL,2,FVAR,IVAR_ID); CALL HANDLE_ERR(IERR)
  IERR = NF_PUT_ATT_TEXT(NCID,IVAR_ID,'long_name',LEN_TRIM(XDESC(IVAR)),TRIM(XDESC(IVAR)))
  CALL HANDLE_ERR(IERR)
  IERR = NF_PUT_ATT_TEXT(NCID,IVAR_ID,'units',LEN_TRIM(XUNIT(IVAR)),TRIM(XUNIT(IVAR)))
  CALL HANDLE_ERR(IERR)
 END DO  ! ivar
 ! define ordinates of probability distributions
 IERR = NF_DEF_VAR(NCID,'ordinates',NF_REAL,1,NORD_DIM,IORD_ID); CALL HANDLE_ERR(IERR)
 IERR = NF_PUT_ATT_TEXT(NCID,IORD_ID,'long_name',37,'ordinates of probability distribution')
 CALL HANDLE_ERR(IERR)
 IERR = NF_PUT_ATT_TEXT(NCID,IORD_ID,'units',1,'-'); CALL HANDLE_ERR(IERR)
 ! define probability distributions
 PVAR = (/NMOD_DIM,NORD_DIM,NPAR_DIM/)   ! dimensions for probability distributions
 IERR = NF_DEF_VAR(NCID,'probability',NF_REAL,3,PVAR,IVAR_ID); CALL HANDLE_ERR(IERR)
 IERR = NF_PUT_ATT_TEXT(NCID,IVAR_ID,'long_name',44,'cumulative probability of number of substeps')
 CALL HANDLE_ERR(IERR)
 IERR = NF_PUT_ATT_TEXT(NCID,IVAR_ID,'units',1,'-'); CALL HANDLE_ERR(IERR)
! end definitions and close file
IERR = NF_ENDDEF(NCID)
! write the ordinates of the probability distribution
rORD = real(ORD_NSUBS,kind(MSP))
IERR = NF_PUT_VAR_REAL(NCID,IORD_ID,rORD); CALL HANDLE_ERR(IERR)     ! write data
IERR = NF_CLOSE(NCID)
! ---------------------------------------------------------------------------------------

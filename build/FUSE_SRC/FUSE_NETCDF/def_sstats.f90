...
IERR = NF_REDEF(ncid_out); CALL HANDLE_ERR(IERR)
 ...
 DO IVAR=1,NSUMVAR
  IERR = NF_DEF_VAR(ncid_out,TRIM(XNAME(IVAR)),NF_REAL,2,FVAR,IVAR_ID); CALL HANDLE_ERR(IERR)
 ...
 IERR = NF_DEF_VAR(ncid_out,'probability',NF_REAL,3,PVAR,IVAR_ID); CALL HANDLE_ERR(IERR)
 IERR = NF_PUT_ATT_TEXT(ncid_out,IVAR_ID,'long_name',44,'cumulative probability of number of substeps')
 CALL HANDLE_ERR(IERR)

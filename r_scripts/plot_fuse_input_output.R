...
dir_input=paste(dir_home,'fuse/input/',sep='')
dir_output=paste(dir_home,'fuse/output/',sep='')

### 3. DEFINE CATCHMENT AND MODEL SETUP 
id<-'08013000' # catchment id
mod_setup<-paste(id,'_FUSE_070__NMETH-2_SSTEP-0_2_0_1.e-2_1.e-2_1.0000000000',sep='') # model setup - string part of the name of the output file

### 4. LOAD FUSE INPUT DATA
input_data<-read.table(paste(dir_input,'input_',id,'.txt',sep=''),sep='',skip=2,header=TRUE)
...
forcing_info_file<-paste(dir_settings,'input_info_',id,'.txt',sep='')
...
fuse_urs<-paste(dir_output,'urs_modrun_',mod_setup,'.nc',sep='') 
